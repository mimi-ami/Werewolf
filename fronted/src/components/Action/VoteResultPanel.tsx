...
export function VoteResultPanel() {
  ...
  return (
    <div className="absolute right-6 top-20 bg-black/45 p-5 rounded-xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur">
      <h3 className="mb-3 text-base uppercase tracking-[0.3em] text-amber-200/80">
        {"\u8ba1\u7968"}
      </h3>
      <div className="space-y-2 text-base">
        ...
        {Object.entries(voteCounts).map(([pid, count]) => (
          <div key={pid} className="flex items-center justify-between gap-6">
            ...
            <span className="rounded-full bg-white/10 px-2 py-0.5 text-sm">
              {count}
            </span>
          </div>
        ))}
